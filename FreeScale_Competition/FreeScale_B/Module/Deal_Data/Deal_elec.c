...
/******电感处理*************
	...
	0     2    1   
**************************/
float WeightCbh01 = 0.34 ,WeightCbh34 = 0.36,WeightOffsetDan = 0.3;
...
/***************************************弯道会车时外径补偿***************************************/
	...
	{
		OffsetDan += 23.0 * (GetADMin[2] - AD[2]) * (GetADMin[2] - AD[2]) / GetADMin[2] / GetADMin[2];
	}
	else if(Enable_Meeting_Car && AD[2] < GetADMin[2])
	{
		OffsetDan += 5.0 * (GetADMin[2] - AD[2]) * (GetADMin[2] - AD[2]) / GetADMin[2] / GetADMin[2];
	}
	L_OffsetDan = OffsetDan;
/*************************************十字************************************************/	
	if(Cross_Flag == 1 && G_AD[5] > 1200)
	{
		Cross_Flag = 2;
	}
...
/****************************弯道会车******************************************/
	...
	{
		ExpectOffset = 0.42 * Cbh01 + 0.18 * Cbh34+ 0.4 * OffsetDan * Dir_E;
	}
/****************************十字******************************************/
...
//	}
	Sum_T =  AD[0] + AD[1] + AD[2];
	if(End_Line_Flag == 0 && Sum_T - R_ADMax > 200 && Roundabout_Asist_Flag == 0 && Sum_T_L <= Sum_T)// && R_D > 8000max01 > 2900 && G_AD[2] > 2900 && Ramp_Flag == 0 && 
	{
		Roundabout_Count++;
	...
	Sum_T_L = Sum_T;	
//	if(End_Line_Flag == 0 && max01 > 2980 && G_AD[2] > 2980 && min01 > 2300 && Roundabout_Asist_Flag == 0 && (L_min01 < min01 || min01 > 2900))//Ramp_Flag == 0 && 
//	{
//		Roundabout_Count++;
//		if(Roundabout_Count > 2)
//		{
...
void Round(void)
{
	...
	else if(Roundabout_Asist_Flag >= 6)
	{
		if(Left_Roundabout_Flag)
		{
			l_sum = G_AD_double[3] * 0.78;
			r_sum = G_AD_double[1];	
		}
		else
		{
			l_sum = G_AD_double[0];
			r_sum = G_AD_double[4] * 0.78;	
		}
		//Round_out();
	}
...
void Round_out(void)
{
	if(Left_Roundabout_Flag)
	{
		l_sum = G_AD_double[0] + G_AD_double[2];
		r_sum = G_AD_double[1] + G_AD_double[4];	
	}
	else
	{
		l_sum = G_AD_double[0] + G_AD_double[3];
		r_sum = G_AD_double[1] + G_AD_double[2];	
	}	
}
